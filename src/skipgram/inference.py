...
class SkipGramInferenceWrapper(mlflow.pyfunc.PythonModel):
    ...
    def predict(self, context, model_input, params=None):
        ...
        len_item = len(self.id_mapping["item_to_index"])
        item_1_indices = [
            self.id_mapping["item_to_index"].get(str(id_), len_item) for id_ in model_input["item_1_ids"]
        ]
        item_2_indices = [
            self.id_mapping["item_to_index"].get(str(id_), len_item) for id_ in model_input["item_2_ids"]
        ]
